...
def add_s(df, supplier, product, contact, email, CEP):
        new_roll = pd.DataFrame({'Supplier':[supplier], 'Produto': [product], 'Contato': [contact], 'E-Mail': [email], 'CEP': [CEP]})
        return pd.concat([df, new_roll], ignore_index=True)

def listar_s(df, product):
    if product in df['Produto'].values:
        print(f"Fornecedor de : {df['Produto'].values}")
